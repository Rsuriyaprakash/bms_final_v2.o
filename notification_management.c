...
BmsStatus_t NotificationManagementEnqueue(BmsNotificationContext_t *c,
...
BmsStatus_t NotificationManagementProcessNext(BmsNotificationContext_t*c,BmsNotificationSender_t sender,void*x){const BmsLinkedListNode_t*p;if((c==NULL)||(sender==NULL))return BMS_STATUS_INVALID_ARGUMENT;for(p=c->history.head;p;p=p->next){BmsNotification_t*n=(BmsNotification_t*)p->data;if(!n->processed){BmsStatus_t s=sender(n,x);if (s == BMS_STATUS_OK)
{
    n->processed = true;
    BMS_LOG_INFO("NOTIFICATION",
...
BmsStatus_t NotificationManagementProcessAll(BmsNotificationContext_t*c,BmsNotificationSender_t sender,void*x,uint32_t*count){const BmsLinkedListNode_t*p;if((c==NULL)||(sender==NULL)||(count==NULL))return BMS_STATUS_INVALID_ARGUMENT;*count=0U;for(p=c->history.head;p;p=p->next){BmsNotification_t*n=(BmsNotification_t*)p->data;if(!n->processed){BmsStatus_t s=sender(n,x);if(s!=BMS_STATUS_OK)return s;n->processed=true;++*count;}}return BMS_STATUS_OK;}
