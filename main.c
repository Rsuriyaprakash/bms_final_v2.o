...
static BmsUiOperationResult_t CompleteDonorProfile(BmsApplication_t *application,
                                                      const BmsUser_t *user);
static BmsUiOperationResult_t UpdateDonorProfile(BmsApplication_t *application,
                                                    const BmsUser_t *user);
...
static void ViewLowStockAlerts(BmsApplication_t *application);
static BmsStatus_t PrintLowStockVisitor(const BmsBloodInventory_t *record,
                                        void *context)
{
    ...
    ++(*count);
    (void)printf("LOW STOCK | Stock ID:%u | Group:%s | Units:%u | Threshold:%u\n",
                 record->bloodId,
...
static void ViewLowStockAlerts(BmsApplication_t *application)
{
    ...
    PrintTitle("LOW-STOCK ALERTS");
    status = BloodInventoryDetectLowStock(&application->inventory,
                                          BMS_LOW_STOCK_THRESHOLD_UNITS,
                                          PrintLowStockVisitor,
                                          &count);
    if (status != BMS_STATUS_OK)
    {
        PrintStatus("Check low-stock inventory", status);
        return;
    }
    if (count == 0U)
    {
        (void)printf("No low-stock blood inventory records.\n");
    }
    else
    {
        (void)printf("Total low-stock records: %u\n", count);
    }
}
...
static void ViewNotifications(const BmsApplication_t *application, const BmsUser_t *user);
static void ViewEmergencyAlerts(const BmsApplication_t *application);
...
static BmsStatus_t NotifyRequestStatusToHospital(
    BmsApplication_t *application,
...
int main(void)
{
    ...
    status = LoggingInitialize();
    if (status != BMS_STATUS_OK)
    {
        (void)printf("Logging initialization: %s\n",
                     UtilityStatusToString(status));
    }
    else
    {
    ...
    if (status == BMS_STATUS_OK)
    {
        ...
        if (status == BMS_STATUS_OK)
        {
            ...
            {
                (void)LoggingWrite(BMS_LOG_LEVEL_DEBUG,
                                   "THREAD",
                                   __func__,
...
static void PrintStatus(const char *operation, BmsStatus_t status)
{
    if (operation != NULL)
    {
        char logMessage[BMS_MAX_MESSAGE_LENGTH + 1U];
        BmsLogLevel_t level = BMS_LOG_LEVEL_INFO;

        (void)printf("%s: %s\n", operation, UtilityStatusToString(status));

        if (status == BMS_STATUS_OK)
        {
            level = BMS_LOG_LEVEL_INFO;
        }
        else if ((status == BMS_STATUS_NOT_FOUND) ||
                 (status == BMS_STATUS_ALREADY_EXISTS) ||
                 (status == BMS_STATUS_INSUFFICIENT_STOCK) ||
                 (status == BMS_STATUS_QUEUE_EMPTY))
        {
            level = BMS_LOG_LEVEL_WARNING;
        }
        else
        {
            level = BMS_LOG_LEVEL_ERROR;
        }

        (void)snprintf(logMessage,
                       sizeof(logMessage),
                       "%s: %s",
                       operation,
                       UtilityStatusToString(status));
        (void)LoggingWrite(level, "BMS", __func__, logMessage);
    }
}
                                    ...
                                    size_t bufferSize)
{
    ...
    if ((prompt != NULL) && (buffer != NULL) && (bufferSize > 1U))
    {
        ...
        BmsStatus_t status = UtilityReadLine(buffer, bufferSize);
        if (status == BMS_STATUS_OK)
        {
            ...
            switch (choice)
            {
                ...
                case 2U:
                {
                    ...
                    if (ready)
                    {
                        ...
                        {
                            (void)LoggingWrite(BMS_LOG_LEVEL_WARNING,
                                               "AUTH",
                                               __func__,
                                               "User login failed");
                                           ...
                                           const BmsUser_t *user)
{
    ...
    while (!done)
    {
        ...
        (void)AuthenticationFindUserById(&application->authentication,
                                         currentUser.userId,
                                         &currentUser);

        PrintTitle("HOSPITAL STAFF MENU");
        (void)printf("Linked Hospital ID: HOS%06lu\n",
                     (unsigned long)currentUser.hospitalId);
        (void)printf(
            "1. Register/link hospital\n"
            "2. Check blood availability\n"
            "3. Create blood request\n"
            "4. List blood requests\n"
            "5. Create emergency alert\n"
            "6. View active emergency alerts\n"
            "7. View notifications / request status\n"
            "8. Logout\n");
        ...
        {
            case 1U:
                if (currentUser.hospitalId != 0U)
                {
                    (void)printf("A hospital is already linked.\n");
                }
                else
                {
                    HANDLE_OPERATION_RESULT(
                        AddHospital(application, currentUser.userId));
                }
                break;

            case 2U:
                HANDLE_OPERATION_RESULT(CheckAvailability(application));
                break;

            case 3U:
                HANDLE_OPERATION_RESULT(
                    CreateBloodRequest(application, &currentUser));
                break;

            case 4U:
                ListRequests(application);
                break;

            case 5U:
                HANDLE_OPERATION_RESULT(
                    CreateEmergencyAlert(application, &currentUser));
                break;

            case 6U:
                ViewEmergencyAlerts(application);
                break;

            case 7U:
                ViewNotifications(application, &currentUser);
                break;
                                           ...
                                           const BmsUser_t *user)
{
    ...
    while (!finished)
    {
        ...
        if (currentUser.linkedDonorId == 0U)
        {
            (void)printf("1. Complete donor profile\n");
        }
        else
        {
            (void)printf("1. Update donor profile\n");
        }
        ...
        switch (choice)
        {
            case 1U:
                ...
                {
                    HANDLE_OPERATION_RESULT(
                        CompleteDonorProfile(application, &currentUser));
                }
                else
                {
                    HANDLE_OPERATION_RESULT(
                        UpdateDonorProfile(application, &currentUser));
                }
                break;
            case 2U:
                HANDLE_OPERATION_RESULT(SearchDonor(application));
                break;
            case 3U:
                ListDonations(application);
                break;
            case 4U:
                ViewDonationCamps(application);
                break;
            case 5U:
                ViewNotifications(application, &currentUser);
                break;
            case 6U:
                HANDLE_OPERATION_RESULT(ChangePassword(application, &currentUser));
                break;
            case 7U:
                finished = true;
                break;
                                                      ...
                                                      const BmsUser_t *user)
{
    ...
    if (user->linkedDonorId != 0U)
    {
        (void)printf("Donor profile already completed. Use Update donor profile.\n");
        return BMS_UI_OPERATION_DONE;
    }
    ...
    PrintTitle("COMPLETE DONOR PROFILE");
    PrintNavigationHelp();
    ...
    while (field < 7U)
    {
        ...
        switch (field)
        {
            ...
            case 1U:
                ...
                if ((result == BMS_UI_VALUE) &&
                    ((age > UINT8_MAX) || (!ValidateAge(age))))
                {
                    (void)printf("Invalid donor age. Enter %u to %u years.\n",
                                 (unsigned int)BMS_MIN_DONOR_AGE,
                                 (unsigned int)BMS_MAX_DONOR_AGE);
                    result = BMS_UI_INVALID;
                }
            ...
            case 2U:
                ...
                if ((result == BMS_UI_VALUE) &&
                    ((weight > UINT16_MAX) || (!ValidateWeight(weight))))
                {
                    (void)printf("Invalid donor weight. Minimum is %u kg.\n",
                                 (unsigned int)BMS_MIN_DONOR_WEIGHT_KG);
                    result = BMS_UI_INVALID;
                }
                else if (result == BMS_UI_VALUE)
                {
            ...
            case 4U:
                ...
                {
                    (void)printf("Invalid phone. Enter exactly 10 digits.\n");
                    result = BMS_UI_INVALID;
                }
                break;
            case 5U:
                result = UiReadText("Email: ", donor.email, sizeof(donor.email));
                if ((result == BMS_UI_VALUE) && (!ValidateEmail(donor.email)))
                {
                    (void)printf("Invalid email address.\n");
                    result = BMS_UI_INVALID;
                }
                break;
            case 6U:
                result = UiReadText("Address: ", donor.address, sizeof(donor.address));
                break;
            default:
                result = BMS_UI_INVALID;
                break;
    ...
    status = DonorManagementAdd(&application->donors, &donor);
    if (status != BMS_STATUS_OK)
    {
        PrintStatus("Complete donor profile", status);
        return BMS_UI_OPERATION_DONE;
    }
    ...
    for (node = application->authentication.users.head;
         ...
         node = node->next)
    {
        BmsUser_t *candidate = (BmsUser_t *)node->data;
        if ((candidate != NULL) && (candidate->userId == user->userId))
        {
    ...
    if (storedUser == NULL)
    {
        (void)DonorManagementDelete(&application->donors, donor.donorId);
        PrintStatus("Link donor profile", BMS_STATUS_NOT_FOUND);
        return BMS_UI_OPERATION_DONE;
    }

    storedUser->linkedDonorId = donor.donorId;
    status = AuthenticationSave(&application->authentication);
    if (status == BMS_STATUS_OK)
    {
        status = DonorManagementSave(&application->donors);
...
{
    BmsDonor_t donor;
    BmsBloodGroup_t originalBloodGroup;
    uint32_t age;
    uint32_t weight;
    uint32_t field = 0U;
    BmsStatus_t status;
    ...
    if (user->linkedDonorId == 0U)
    {
        (void)printf("Donor profile is not completed yet.\n");
        return BMS_UI_OPERATION_DONE;
    }

    status = DonorManagementSearchById(&application->donors,
                                        user->linkedDonorId,
                                        &donor);
    if (status != BMS_STATUS_OK)
    {
        PrintStatus("Find donor profile", status);
        return BMS_UI_OPERATION_DONE;
    }

    originalBloodGroup = donor.bloodGroup;
    age = (uint32_t)donor.age;
    weight = (uint32_t)donor.weightKg;

    PrintTitle("UPDATE DONOR PROFILE");
    (void)printf("Blood group: %s (locked - cannot be changed)\n",
    ...
    donor.bloodGroup = originalBloodGroup;
    status = DonorManagementUpdate(&application->donors, &donor);
    if (status == BMS_STATUS_OK)
    {
        status = DonorManagementSave(&application->donors);
    }

    PrintStatus("Update donor profile", status);
    return BMS_UI_OPERATION_DONE;
}
...
static BmsUiOperationResult_t SearchDonor(BmsApplication_t *application)
{
    ...
    if (choice == 4U)
    {
        ...
        {
            const BmsDonor_t *donor = (const BmsDonor_t *)node->data;
...
static BmsUiOperationResult_t CreateBloodRequest(
    BmsApplication_t *application,
    const BmsUser_t *requester)
{
    ...
    if ((application == NULL) || (requester == NULL))
    {
        return BMS_UI_OPERATION_MENU;
    }

    if (requester->hospitalId == 0U)
    {
        (void)printf("No hospital is linked. Register a hospital first.\n");
        return BMS_UI_OPERATION_DONE;
    }

    if (HospitalManagementSearchById(&application->hospitals,
                                     requester->hospitalId,
                                     &hospital) != BMS_STATUS_OK)
    {
    ...
    request.requesterHospitalId = requester->hospitalId;
    ...
    while (field < 5U)
    {
        ...
        {
            case 0U:
                result = UiReadBloodGroup(&request.bloodGroup);
                break;
            ...
            case 2U:
                ...
                if ((result == BMS_UI_VALUE) && (priority <= 3U))
                {
                    request.priority = (BmsPriority_t)priority;
                }
                else if (result == BMS_UI_VALUE)
                {
                    result = BMS_UI_INVALID;
            ...
            case 3U:
                result = UiReadDate("Request date (YYYY-MM-DD): ",
                                    &request.requestDate);
                break;

            case 4U:
                result = UiReadDate("Need-by date (YYYY-MM-DD): ",
    ...
    PrintStatus("Create request", status);

    if (status == BMS_STATUS_OK)
    {
        (void)NotifyRequestStatusToHospital(
            application,
            &request,
            "PENDING - waiting for blood bank processing");
        (void)ApplicationSave(application);
...
static void ProcessNextRequest(BmsApplication_t *application)
{
    ...
    if (status == BMS_STATUS_OK)
    {
        (void)EmergencyAlertManagementResolveByRequestId(
            &application->alerts,
            request.requestId);
        ...
        (void)NotifyRequestStatusToHospital(
            application,
            &request,
            "FULFILLED - accepted and completed by blood bank");

        (void)ApplicationSave(application);

        if (PrintRequestVisitor(&request, NULL) != BMS_STATUS_OK)
        {
            (void)printf("Unable to display processed request.\n");
        }
    }
    else if (status == BMS_STATUS_INSUFFICIENT_STOCK)
    {
        (void)NotifyRequestStatusToHospital(
            application,
            &request,
            "PENDING - insufficient stock, waiting for availability");

        (void)ApplicationSave(application);

        if (PrintRequestVisitor(&request, NULL) != BMS_STATUS_OK)
        {
            (void)printf("Unable to display pending request.\n");
        }
    }
}
    ...
    const char *statusText)
{
    ...
    (void)memset(&notification, 0, sizeof(notification));
    notification.notificationId = NextNotificationId(application);
    notification.recipientUserId = 0U;
    notification.recipientHospitalId = request->requesterHospitalId;
    notification.channel = BMS_NOTIFICATION_CHANNEL_CONSOLE;
    ...
    (void)snprintf(
        notification.message,
        sizeof(notification.message),
        "Blood request REQ%06lu: %s | %u units %s | fulfilled %u/%u",
        (unsigned long)request->requestId,
        statusText,
        request->requestedUnits,
    ...
    return NotificationManagementEnqueue(&application->notifications,
                                         &notification);
}
    ...
    const BmsUser_t *user)
{
    ...
    (void)memset(&request, 0, sizeof(request));
    (void)memset(&requiredByDate, 0, sizeof(requiredByDate));
    ...
    (void)printf(
        "Alert ID: ALT%06lu | Request ID: REQ%06lu | "
    ...
    (void)NotifyRequestStatusToHospital(
        application,
        &request,
        "PENDING EMERGENCY - sent to blood bank staff");
...
static void ViewEmergencyAlerts(const BmsApplication_t *application)
{
    ...
    PrintTitle("ACTIVE EMERGENCY ALERTS");

    for (node = application->alerts.alertHistory.head;
         node != NULL;
         node = node->next)
    {
        const BmsEmergencyAlert_t *alert =
        ...
        if ((alert != NULL) && (!alert->resolved))
        {
            ++activeCount;
            (void)printf(
                "ALT%06u | REQ%06u | HOS%06u | %s | "
                "Units:%u | ACTIVE | %s\n",
                alert->alertId,
                alert->requestId,
                alert->sourceHospitalId,
                BloodGroupToString(alert->bloodGroup),
                alert->requiredUnits,
                alert->message);
        }
    ...
    if (activeCount == 0U)
    {
        (void)printf("No active emergency alerts.\n");
    }
}
