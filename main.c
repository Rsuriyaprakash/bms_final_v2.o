...
static BmsUiOperationResult_t CreateBloodRequest(BmsApplication_t *application, const BmsUser_t *requester)
{
    ...
    while (field < 5U)
    {
        ...
        switch (field)
        {
            ...
            case 4U: result = UiReadDate("Need-by date (YYYY-MM-DD): ", &request.requiredByDate); break;
            default: break;
        }
        HANDLE_FIELD_RESULT(result, field);
    }
    ...
    const BmsUser_t *user)
{
    ...
    (void)LoggingWrite(BMS_LOG_LEVEL_INFO,
                       "REQUEST",
                       __func__,
                       "Pending emergency blood request created");
...
static void ViewEmergencyAlerts(const BmsApplication_t *application){const BmsLinkedListNode_t*n;if(application==NULL)return;PrintTitle("EMERGENCY ALERTS");if(application->alerts.alertHistory.count==0U){(void)printf("No emergency alerts.\n");return;}for(n=application->alerts.alertHistory.head;n;n=n->next){const BmsEmergencyAlert_t*a=(const BmsEmergencyAlert_t*)n->data;(void)printf("ALT%06u | HOS%06u | %s | Units:%u | %s | %s\n",a->alertId,a->sourceHospitalId,BloodGroupToString(a->bloodGroup),a->requiredUnits,a->resolved?"RESOLVED":"ACTIVE",a->message);}}
