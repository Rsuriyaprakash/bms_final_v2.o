...
static void test_hospital_missing_records(void)
{
    BmsHospitalContext_t context;
    BmsHospital_t out;
    ...
    CU_ASSERT_EQUAL(HospitalManagementInitialize(&context), BMS_TEST_SUCCESS);
    CU_ASSERT_NOT_EQUAL(HospitalManagementSearchById(&context, 999999U, &out),
                        BMS_TEST_SUCCESS);
    CU_ASSERT_NOT_EQUAL(HospitalManagementDelete(&context, 999999U),
                        BMS_TEST_SUCCESS);
    CU_ASSERT_NOT_EQUAL(HospitalManagementUpdate(&context, &out),
                        BMS_TEST_SUCCESS);

    HospitalManagementDeinitialize(&context);
}
...
void RegisterHospitalTests(void)
{
    ...
    (void)CU_add_test(suite,
                      "Hospital Missing Records",
                      test_hospital_missing_records);
}
