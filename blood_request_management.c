...
BmsStatus_t BloodRequestManagementFulfill(BmsBloodRequestContext_t*c,BmsInventoryContext_t*i,BmsRequestId_t id)
{void*f=NULL;
BmsLinkedListNode_t*n;
...
BmsStatus_t BloodRequestManagementProcessNext(BmsBloodRequestContext_t*c,BmsInventoryContext_t*i,BmsBloodRequest_t*out){const BmsLinkedListNode_t*n;BmsBloodRequest_t*best=NULL;BmsStatus_t s;if((c==NULL)||(i==NULL)||(out==NULL))return BMS_STATUS_INVALID_ARGUMENT;for(n=c->requests.head;n;n=n->next){BmsBloodRequest_t*r=(BmsBloodRequest_t*)n->data;if((r->status==BMS_REQUEST_STATUS_PENDING)||(r->status==BMS_REQUEST_STATUS_PROCESSING)){if((best==NULL)||Better(r,best))best=r;}}if (best == NULL)
{
